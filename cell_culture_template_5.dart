...
class _CellCultureTemplatePageState extends State<CellCultureTemplatePage> {
  ...
  List<List<String>> generatePlateLayout({
    ...
    // Place controls from the last column first, top to bottom.
    int controlIndex = 0;
    ...
    // Fill samples into remaining empty wells from front.
    int sampleIndex = 0;
    for (int r = 0; r < rows; r++) {
...
// 좋아요. 아래에 smarter control layout + legend 포함 최종본으로 한 번에 정리해드릴게요.

// 바뀐 핵심:

// control을 마지막 열부터 우선 배치

// control 순서: BLK → NC → VEH → PC

// legend 추가

// sample은 control이 차지하지 않은 나머지 well에 앞에서부터 자동 배치